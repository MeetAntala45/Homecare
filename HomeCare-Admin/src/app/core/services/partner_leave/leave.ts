...
} from '../../../core/models/partner_leave/leave.js';
...
export class LeaveService {
  ...
  getAllLeaves(filter: LeaveFilter): Observable<ApiResponse<PagedResult<AdminLeaveItem>>> {
    let params = new HttpParams()
      .set('pageNumber', filter.pageNumber ?? 1)
      .set('pageSize', filter.pageSize ?? 10);
    ...
    if (filter.partnerName) {
      params = params.set('partnerName', filter.partnerName);
    }

    return this.http.get<ApiResponse<PagedResult<AdminLeaveItem>>>(this.adminBase, { params });
  }
