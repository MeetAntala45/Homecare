...
import { PartnerSystemNotif, ApiResponse } from '../../../core/models/servicepartner_leave/leave.js';
import { API_BASE_URL } from '../../constants/environment-config.js';

@Injectable({ providedIn: 'root' })
export class PartnerSystemNotification {

  private hub!: HubConnection;

  private readonly base = `${API_BASE_URL}/api/partner/system-notifications`;
  private readonly hubUrl = `${API_BASE_URL}/hubs/booking`;

  unreadCount = signal<number>(0);
  notifications = signal<PartnerSystemNotif[]>([]);
  ...
  async connectSignalR(token: string): Promise<void> {
    ...
    this.hub.on('ReceiveSystemNotification', (notif: PartnerSystemNotif) => {
  ...
  loadAll(): void {
    this.http.get<ApiResponse<PartnerSystemNotif[]>>(this.base)
