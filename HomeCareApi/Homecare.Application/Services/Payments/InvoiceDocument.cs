...
public class InvoiceDocument : IDocument
{
    ...
    private readonly decimal _refereeDiscount;
    private readonly decimal _walletDiscount;
    ...
    public InvoiceDocument(
        ...
        decimal refereeDiscount,
        decimal walletDiscount)
    {
        _booking = booking;
        _service = service;
        _payment = payment;
        ...
        _refereeDiscount = refereeDiscount;
        _walletDiscount = walletDiscount;
    }
        ...
        c.Column(col =>
        {
            col.Item().Row(row =>
            {
                ...
                row.RelativeItem().AlignRight().AlignMiddle()
                    .Text("INVOICE").FontSize(28).Bold().FontColor(Primary);
            });
    ...
    void ComposeServiceTable(IContainer c)
    {
        c.Table(table =>
        {
            ...
            var normalisedName = System.Globalization.CultureInfo.CurrentCulture.TextInfo
                .ToTitleCase(_service.Name.Trim().ToLower());

            table.Cell().Element(DataCell).Column(inner =>
            {
                inner.Item().Text(normalisedName).Bold();
            ...
            table.Cell().Element(DataCell).Column(inner =>
            {
                if (_partnerName != null)
                {
                    ...
                    {
                        text.Span(_partnerName).Bold().FontSize(10).FontColor(DarkText);
                        if (!string.IsNullOrEmpty(_partnerMobileNumber))
                        {
                            text.Span("  ");
                            text.Span($"({_partnerMobileNumber})").FontSize(9).FontColor(GrayText);
                        }
                    });
    ...
    {
        c.AlignRight().Width(95, Unit.Millimetre).Column(col =>
        {
            ...
            {
                var label = string.IsNullOrWhiteSpace(_couponCode)
                    ? "Coupon Discount"
                    : $"Coupon ({_couponCode})";
                TotalRow(col, label, $"-${_couponDiscount:F2}", valueColor: Green);
            }

            if (_refereeDiscount > 0)
            {
                TotalRow(col, "First Order — 50% Off", $"-${_refereeDiscount:F2}",
                    valueColor: Green);
            }

            if (_walletDiscount > 0)
            {
                TotalRow(col, "Wallet Credit", $"-${_walletDiscount:F2}",
                    valueColor: Green);
            }
    ...
    void ComposeBookingDetails(IContainer c)
    {
        ...
        var addressFull = $"{_address.HouseFlatNo}, {_address.Landmark}, {_address.DisplayName}";
    ...
    void DetailRow(TableDescriptor table, string label, string value)
    {
        static IContainer Cell(IContainer c) =>
            c.Background("#F5F5FF").BorderBottom(0.3f).BorderColor("#E5E7EB").Padding(8);
    ...
    void ComposeFooter(IContainer c)
    {
        c.Column(col =>
        {
            ...
            {
                r.RelativeItem().AlignRight()
                    .Text("Thank you for choosing HomeCare!")
                    .FontSize(8).FontColor(GrayText);
            });
        });
    }
